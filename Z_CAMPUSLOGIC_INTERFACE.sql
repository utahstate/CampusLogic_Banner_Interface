create PACKAGE BODY          z_campuslogic_interface
AS
    /****************************************************************************
      NAME:       z_campuslogic
    ...
    ****************************************************************************/
    ...
    (
       ...
       ZCLELOG_TRANSACTIONCATEGORYID   INTEGER,
       ZCLELOG_EVENTNOTIFICATIONID     INTEGER,
       ZCLELOG_DOCUMENTNAME            VARCHAR2 (100 CHAR),
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                ...
                rp_applicant_status.p_update (
                    p_aidy_code     => p_awardYear,
                    p_pidm          => p_pidm,
    ...
    */
    PROCEDURE p_update_xtender (
        ...
        p_treqCode             rrrareq.rrrareq_p_treqCode%TYPE,
        p_status               VARCHAR2,
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                p_tracking_upd_api (pidm_in        => p_studentPidm,
                                    aidy_in        => p_awardYear,
                                    p_treqCode_in   => p_treqCode,
                                    p_status      => p_status,
                                    sys_ind_in     => 'B');
            END IF;
        END IF;
    ...
    */
    ...
    AS
        ...
        status_in                         VARCHAR2 (1);
        treq_code                         rrrareq.rrrareq_treq_code%TYPE;
        ...
        v_banner_pj_code         CONSTANT rrrareq.rrrareq_treq_code%TYPE
                                              := 'PROJUD' ;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                DBMS_OUTPUT.PUT_LINE('ERROR: failed to insert event notification into ZCLELOG');
        END;
        ...
        EXCEPTION
            ...
            WHEN TOO_MANY_ROWS THEN
                DBMS_OUTPUT.PUT_LINE('ERROR: award year is invalid: ' || p_awardYear);
        END;
        ...
        CASE
            ...
            THEN                          --209 is Account Created (see notes)
                UPDATE ROBNYUD
                   SET ROBNYUD_ACTIVITY_DATE = SYSDATE,
                       ROBNYUD_VALUE_3 = 'ACCTCL'
                 WHERE robnyud_pidm = v_student_pidm;
            WHEN (p_eventNotificationId = 103)
            THEN                               --103 is File Review(see notes)
                IF (NVL (TransactionCategoryId, 0) = 1) --Student Verification
                THEN
                    p_tracking_upd_api (
                        pidm_in        => v_student_pidm,
                        aidy_in        => v_aidy_code,
                        treq_code_in   => v_banner_verify_code,
                        status_in      => 'Z',                  --from RTVTRST
                        sys_ind_in     => 'B');
                END IF;
            ...
            THEN                                --104 is Correction(see notes)
                ...
                THEN
                    p_tracking_upd_api (
                        ...
                        aidy_in        => v_aidy_code,
                        treq_code_in   => v_banner_verify_code,
                        status_in      => 'Q',                  --from RTVTRST
                        sys_ind_in     => 'B');
                END IF;
            WHEN (p_eventNotificationId = 105 AND DocumentName IS NULL)
            THEN                    --105 is Transaction Completed (see notes)
                CASE
                    WHEN NVL (TransactionCategoryId, 0) = 1 --Student Verification
                    THEN            --update Banner RRRAREQ verify requirement
                        p_tracking_upd_api (
                            pidm_in        => v_student_pidm,
                            aidy_in        => v_aidy_code,
                            treq_code_in   => v_banner_verify_code,
                            status_in      => 'C',              --from RTVTRST
                            sys_ind_in     => 'B');
                        p_status_upd_api (pidm_in           => v_student_pidm,
                                          aidy_in           => AwardYear,
                                          ver_pay_ind_in    => 'V',
                                          ver_complete_in   => 'Y');
                    WHEN NVL (TransactionCategoryId, 0) = 2       --SAP Appeal
                    THEN               --update Banner RRRAREQ SAP requirement
                        p_tracking_upd_api (
                            pidm_in        => v_student_pidm,
                            aidy_in        => v_aidy_code,
                            treq_code_in   => v_banner_sap_code,
                            status_in      => 'C',              --from RTVTRST
                            sys_ind_in     => 'B');
                    WHEN NVL (TransactionCategoryId, 0) IN (3, 4) --PJ Dependency Override Appeal
                    THEN                --update Banner RRRAREQ PJ requirement
                        p_tracking_upd_api (
                            pidm_in        => v_student_pidm,
                            aidy_in        => v_aidy_code,
                            treq_code_in   => v_banner_pj_code,
                            status_in      => 'C',              --from RTVTRST
                            sys_ind_in     => 'B');
                END CASE;
            ...
            THEN
                ...
                -- 701 is Scholarship Award Posted
                IF p_suPostType = 'Add' THEN
                    -- If suPostType is Add call the award schedule create API from banner
                    rp_award_schedule.p_create(p_aidy_code          => v_aidy_code,
                                               p_pidm               => v_student_pidm,
                                               p_fund_code          => p_suScholarshipCode,
                                               p_term_code          => v_term,
                                               p_offer_amt          => p_suAmount,
                                               p_offer_date         => to_date(substr(p_eventDateTime,1,length(p_eventDateTime)-3),'MM/DD/YYYY HH24:MI:SS'),
                                               p_accept_amt         => p_suAmount,
                                               p_accept_date        => to_date(substr(p_eventDateTime,1,length(p_eventDateTime)-3),'MM/DD/YYYY HH24:MI:SS'));
                END IF;
        END CASE;
        ...
        THEN
            ...
            THEN
                ...
                IF (p_sfDocumentName IS NOT NULL AND status_in IS NOT NULL)
                ...
                THEN
                    ...
                    SELECT COUNT (*)
                     ...
                     WHERE     zclxwlk_aidy_code = v_aidy_code
                           AND UPPER (zclxwlk_document) =
                               UPPER (p_sfDocumentName)
                    ...
                    THEN
                        SELECT zclxwlk_treq_code
                         ...
                         WHERE     zclxwlk_aidy_code = v_aidy_code
                               AND UPPER (zclxwlk_document) =
                                   UPPER (p_sfDocumentName)
                               AND NVL (zclxwlk_treq_code, 'MINFO') <>
                                   'MINFO';
                    ELSE
                        treq_code := NULL;
                    END IF;
